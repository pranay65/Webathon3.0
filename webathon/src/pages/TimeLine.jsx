  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        {request && (
          <div className="mb-4">
            <p className="text-lg font-semibold">Project Name: {request.name}</p>
          ...
          {timelineSteps.map((step, index) => (
            <div key={index} className="ml-4 relative">
              ...
              <div className="bg-gray-50 p-4 rounded-md shadow">
                <p className={`text-lg font-medium ${step.done ? "text-green-700" : "text-gray-700"}`}>
                  {step.step}
                </p>
                <p className="text-sm text-gray-500">{step.date}</p>
                <Link to="/contract">
                <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg mt-2">Contract</button>
                </Link>
              </div>
            </div>
          ))}
